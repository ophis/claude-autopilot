...
- **E2 — brainstorm (step 2).** Use `superpowers:brainstorming` on `$ARGUMENTS` →
  ...
  defaults: decide + record.) (**Spec-file mode:**
...
- **S3 — produce (step 5).** Produce the work product. Code →
  `superpowers:subagent-driven-development`: keep its per-task reviews (early-catch),
  SKIP its final whole-implementation review — S5 is the authoritative whole-diff gate
  that re-reviews the same diff. It may commit per task; the S6 squash folds its
  commits. Non-code → producer subagents via the same dispatch pattern. The
