...
allowed-tools: Bash, Read, Write, Edit, Glob, Grep, Task, SendMessage, Skill, ToolSearch, EnterWorktree, ExitWorktree, TodoWrite
---
...
**re-run from scratch** (re-dispatch the whole frozen panel — bounded — continuing recorded
agent IDs where reachable; after a resumed session expect the fallback (fresh + gists)), only
...
re-review round). The orchestrator runs the loop itself, dispatching each round via `Task`,
re-reviews by continuation.
...
- **Dispatch each round together** — never one at a time, re-reviews included. Build each
  ...
  reviewers read the worktree, never main).
...
- **The loop** (orchestrator-run, cap = 1):
  ...
  - **Fix** (orchestrator-driven): the first fix dispatches ONE fresh producer subagent primed
    with the deduped open blockers (with item IDs) + cited files only; later fixes continue it
    via `SendMessage` (unknown ID / error → fresh producer, whose ID replaces the recorded one). It returns the claimed-fixes mapping
    for every change it made, incl. non-blockers fixed opportunistically (worktree-pinned — see
    Operating disciplines). A fix-time
  ...
  - **Re-review** (the one round cap = 1 allows) dispatches only **`(FAILed ∪ touched) ∩ frozen
    ...
    lenses re-run iff FAILed. Skipped lenses carry their PASS. Diff reference:
